...
public class Control implements ActionListener {
    ...
    /**
     * This method deals with all the ActionCommands and allows for interactivity
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getActionCommand().equals("quit")) {
            ...
            try {
                Event a = new Event(win.getAddEvent().getField().getText(), getUser(),
                        LocalDate.parse(win.getAddEvent().getField3().getText()),
                        LocalTime.parse(win.getAddEvent().getField4().getText()),
                        win.getAddEvent().getField5().getText(),
                        Integer.parseInt(win.getAddEvent().getField6().getText()));
                eventManager.addEvent(a);
            } catch (Exception ev) {
                JOptionPane.showMessageDialog(win.getFrame(), "One of the fields has not been entered correctly",
            ...
            for (Event et : ep.keySet()) {
                if (ep.get(et).getCheckBox().isSelected()) {
                    ...
                    try {
                        Agenda ag = new Agenda(win.getAddAgenda().getField().getText(),
                                LocalTime.parse(win.getAddAgenda().getField2().getText()));
                        eventManager.addItem(ag);
                        eventManager.addAgendaItem(et, ag);

                    } catch (Exception ev) {
                        JOptionPane.showMessageDialog(win.getFrame(),
                                "One of the fields has not been entered correctly", "Error", JOptionPane.ERROR_MESSAGE);
                        Agenda ag = new Agenda(win.getAddAgenda().getField().getText(),
                                LocalTime.parse(win.getAddAgenda().getField2().getText()));
                        eventManager.addItem(ag);
                        eventManager.addAgendaItem(et, ag);
            ...
            for (Event et : ep.keySet()) {
                if (ep.get(et).getCheckBox().isSelected()) {
                    win.getCon().getEvent().setText("Title: " + et.getTitle() + System.lineSeparator() + "Oragniser: "
                            + et.getOrganiser().toString() + System.lineSeparator() + "Date: " + et.getDate()
                            + System.lineSeparator() + "Time: " + et.getTime() + System.lineSeparator() + "Location: "
                            + et.getLocation() + System.lineSeparator() + "Capacity: " + et.getCapacity());
            ...
            outer: for (Event et : ep.keySet()) {
                if ((ep.get(et).getCheckBox().isSelected()) && (agenda == false)) {
                    eventManager.updateEvent(et, win.getEditEvent().getField().getText(), getUser(),
                            win.getEditEvent().getField3().getText(), win.getEditEvent().getField4().getText(),
                            win.getEditEvent().getField5().getText(),
                            Integer.parseInt(win.getEditEvent().getField6().getText()));
            ...
            if (ep.get(et).getCheckBox().isSelected()) {
                if (et.getAgenda().size() > 0) {
                    ...
                    for (int i = 0; i < et.getAgenda().size(); i++) {
                        AgendaPanel apinstance = new AgendaPanel(et.getAgenda().get(i).getTitle(),
                                et.getAgenda().get(i).getTime().toString());
